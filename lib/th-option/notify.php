...
class ThemeHunk_Child_Notify{
	...
	function notify(){
        ...
        printf( esc_html__( 'Welcome to %1$s - Version %2$s', 'open-mart' ), esc_html( $my_theme->Name ), esc_html( $my_theme->Version ) );
        ?>
      </h1>

        <p>
        <?php printf( esc_html__( 'Get Started with %1$s and Start customizing your website, also Install the Child theme if you want to edit the core code of the theme.', 'open-mart' ), esc_html( $my_theme->Name ));?>
        ...
        <a href="<?php echo esc_url(admin_url('themes.php?page=thunk_started')); ?>" class="button button-primary th-blue">
        	<?php
               ...
               printf( esc_html__( 'Get Started with %1$s', 'open-mart' ), esc_html( $my_theme->Name ));

              ?>	
        </a>

        <a href="<?php echo esc_url(admin_url('themes.php?page=thunk_started&th-tab=get-child-theme')); ?>" class=" button-secondary"><?php _e("Get Child Theme Now","open-mart") ?></a>
