(function($){
    ...
    OpenMartAdmin = {
        ...
        _activetedPlugin: function(event, args){
                event.preventDefault();
                var $card = jQuery( '.'+args.slug);
                OpenMartAdmin._activePluginHomepage(args.slug,$card.data('init'));
        },
        /**
        ...
        _activePluginHomepage: function($slug,$init){
            ...
            if($init=='hunk-companion/hunk-companion.php' || $init=='one-click-demo-import/one-click-demo-import.php'|| $init=='woocommerce/woocommerce.php'){
                     if(jQuery('button.hunk-companion').hasClass('disabled') && jQuery('button.one-click-demo-import').hasClass('disabled') && jQuery('button.woocommerce').hasClass('disabled')){ 
                            var output = '<a href="'+ openmart.openmartSitesLink +'" aria-label="'+ openmart.openmartSitesLinkTitle +'">' + openmart.openmartSitesLinkTitle +' </a>';
        ...
        _activePlugin: function(event){
                ...
                OpenMartAdmin._activePluginHomepage($slug,$init);
            },
        _bind: function(){               
            $( document ).on('click'                     , '.install-now', OpenMartAdmin._installNow);
            $( document ).on('click'                     , '.activate-now', OpenMartAdmin._activePlugin);
            $( document ).on('wp-plugin-install-error'   , OpenMartAdmin._installError);
            $( document ).on('wp-plugin-installing'      , OpenMartAdmin._pluginInstalling);
            $( document ).on('wp-plugin-install-success' , OpenMartAdmin._activetedPlugin);  
        },
