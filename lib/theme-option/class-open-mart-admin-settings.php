    ...
    /**
		...
		static public function init_admin_settings(){
            ...
            add_action( 'open_mart_welcome_page_main_content', __CLASS__ . '::open_mart_recommend_plugins',10 );
             ...
             add_action( 'open_mart_welcome_page_main_content', __CLASS__ . '::open_mart_useful_plugins',10 );
			...
			add_action( 'open_mart_useful_plugins_setup', __CLASS__ . '::open_mart_useful_plugin_setup_api',17);
		...
		static public function styles_scripts(){
			...
			$localize = array(
				...
				'openmartSitesLink'      => esc_url(admin_url( 'themes.php?page=pt-one-click-demo-import' )),
				'openmartSitesLinkTitle' => __( 'See Library', 'open-mart' ),
			);
			wp_localize_script( 'open-mart-admin-settings', 'openmart', apply_filters( 'open_mart_theme_js_localize', $localize ) );
		}
		...
		static public function open_mart_recommend_plugins(){	
			?>
			<div id="open-mart-recommend-plugins" class="postbox open-mart-recommend-plugins panel">
				<div class="inside">
		 ...
		 *
		 */
		static public function open_mart_useful_plugins(){	
			?>
			<div id="open-mart-useful-plugins" class="postbox open-mart-useful-plugins panel">
				<div class="inside">
					<?php do_action( 'open_mart_useful_plugins_setup' ); ?>
			    </div>
			</div>
			<?php } 
		...
		static public function open_mart_welcome_page_five_star_scetion(){
			...
			<div class="postbox">
				...
				<div class="inside">
					...
					<?php
						$open_mart_support_link       = apply_filters( 'open_mart_support_link','https://www.support.themehunk.com/');
       ...
       if ( is_array( $recommend_plugins ) && isset( $recommend_plugins[0] ) ){
        foreach($recommend_plugins[0] as $slug=>$plugin){
            ...
            if ( ! is_plugin_active( $active_file_name ) ){
        		...
        		echo'<img src="'.esc_url(OPEN_MART_THEME_URI. 'lib/theme-option/assets/images/th-advance-product-search.png' ).'" />';	 
                $detail= '';
                }elseif($plugin_slug=='th-variation-swatches'){
        		echo'<img src="'.esc_url(OPEN_MART_THEME_URI. 'lib/theme-option/assets/images/th-variation-swatches.png' ).'" />'; 	
                $detail='';
                }elseif($plugin_slug=='woo-smart-wishlist'){
                	echo'<img src="'.esc_url(OPEN_MART_THEME_URI. 'lib/theme-option/assets/images/wishlist.png' ).'" />'; 
                $detail='';
                }elseif($plugin_slug=='woo-smart-compare'){
...
/**
		 * Include Welcome page content
		 */
       static public  function open_mart_useful_plugin_setup_api(){
       include_once( ABSPATH . 'wp-admin/includes/plugin-install.php' );
       network_admin_url( 'plugin-install.php' );
